...
ready(function() {
  ...
  function initDatGui() {
    ...
    secretFolder
      .add(setupObj, "time", 0.0, 1.0)
      .listen()
      .onChange(safeRedraw);
    secretFolder.add(setupObj, "drawNumbers").onChange(safeRedraw);
    secretFolder
      .add(setupObj, "compositeMode", compositeTypes)
      .onChange(safeRedraw);
    secretFolder
      .add(setupObj, "renderMethod", ["single", "full"])
      .onChange(safeRedraw);
  }
  ...
  function getCurrentPoints() {
    ...
    const setPoints = currentModel
      .map(triangleGrid)
      .map(fn => fn(setupObj.cubeDistance * setupObj.cubeWidth));

    // Movement percentage
    const floatTimePoints = setPoints.map((pnt, ind, arr) =>
      getLineXYatPercent(pnt, arr[(ind + 1) % arr.length], setupObj.time)
    );
  ...
  function draw() {
    ...
    if (setupObj.renderMethod == "single") drawMethSingle.draw(toPass);
    else drawMethFull.draw(toPass);
